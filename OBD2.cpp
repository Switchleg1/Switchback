...
float OBD2Class::pidRead(uint8_t pid)
{
  ...
  switch (pid) {
    ...
    case OXYGEN_SENSOR_1_FUEL_AIR_RATIO:
    case OXYGEN_SENSOR_2_FUEL_AIR_RATIO:
    case OXYGEN_SENSOR_3_FUEL_AIR_RATIO:
    case OXYGEN_SENSOR_4_FUEL_AIR_RATIO:
    case OXYGEN_SENSOR_5_FUEL_AIR_RATIO:
    case OXYGEN_SENSOR_6_FUEL_AIR_RATIO:
    case OXYGEN_SENSOR_7_FUEL_AIR_RATIO:
    case OXYGEN_SENSOR_8_FUEL_AIR_RATIO:
...
{
  // make sure at least OBD2_RESPONSE_TIME ms have passed since the last response
  unsigned long lastResponseDelta = millis() - _lastPidResponseMillis;
  if (lastResponseDelta < OBD2_RESPONSE_TIME) {
    delay(OBD2_RESPONSE_TIME - lastResponseDelta);
  }
...
//OBD2 & PID
volatile uint16_t   OBD2Started;
volatile uint16_t   OBD2Print;
uint16_t            updatePosition;
uint16_t            pidReadPosition;
uint16_t            pidWritePosition;
uint32_t            lastPidMillis;
volatile uint32_t   lastPidTime;
volatile float      PIDValues[NUM_PIDS];
...
#ifdef EEPROM_RESET
  EEPROMWriteByte(EEPROM_PRINT_PIDS, true);
#endif

  //Read EEPROM settings
  OBD2Print = EEPROMReadByte(EEPROM_PRINT_PIDS);

  LogPrintln("complete.");
}

void OBD2Loop(void* parameter)
{
  LogPrintln("OBD2 task: started.");
  //Its time to update the next PID
  for(;;)
  {
#ifdef DEBUG_TASK
    LogPrintln("OBD2 task: running.");
#endif
  
      //Check OBD2 connection
      if(OBD2Started == 2)
      {
        //Keep scanning until we read a PID
        uint16_t readPID = false;
        while(!readPID)
        {
          //read the current PID value
          if(updatePosition % PIDS[pidReadPosition][1] == 0)
          {
            PIDValues[pidReadPosition] = OBD2.pidRead(PIDS[pidReadPosition][0]);
            readPID = true;
          }

          //Check if we have completed the last PID in the list
          if(++pidReadPosition >= NUM_PIDS)
          {
            pidReadPosition = 0;
      
            //increment the updatePosition
            updatePosition++;
          }
        }

        //are we writing PIDS and sensors to the log?
        if(OBD2Print)
        {
          //If not first PID add a comma
          if(pidWritePosition)
            LogPrint(",");

          //Are we writing PIDS or Sensors?
          if(pidWritePosition < NUM_PIDS)
          {
            if(isnan(PIDValues[pidWritePosition])) LogPrint("0");
              else LogPrint(PIDValues[pidWritePosition]);
          } else if(pidWritePosition < NUM_PIDS+4)
          {
            switch(pidWritePosition-NUM_PIDS)
            {
              case 0:
                LogPrint(FindSensorTableValue(Sensors[SENSOR_BOOST].raw, SEN_TABLE_PUT));
                LogPrint(",");
                LogPrint(FindSensorTableValue(Sensors[SENSOR_BOOST].value, SEN_TABLE_PUT));
                break;
              case 1:
                LogPrint(FindSensorTableValue(Sensors[SENSOR_MANIFOLD].raw, SEN_TABLE_MAP));
                LogPrint(",");
                LogPrint(FindSensorTableValue(Sensors[SENSOR_MANIFOLD].value, SEN_TABLE_MAP));
                break;
              case 2:
                LogPrint(Sensors[SENSOR_FUEL].raw);
                LogPrint(",");
                LogPrint(Sensors[SENSOR_FUEL].value);
                break;
              case 3:
                LogPrint(Sensors[SENSOR_O2].raw);
                LogPrint(",");
                LogPrintln(EngineBoostAverage());
                break;
            }
          }
        
          //update write position and return to 0 if past max PIDS + sensors
          if(++pidWritePosition >= NUM_PIDS+4)
          {
            pidWritePosition = 0;
          }
        } else
        {
          //If we aren't writing return to 0 position and get ready
          pidWritePosition = 0;
        }

        //store millis time
        lastPidTime = millis() - lastPidMillis;
        lastPidMillis = millis();
      } else
      {  //OBD2 not connected, try to connect
        OBD2Started = 1;
        if(OBD2.begin())
        {
          lastPidMillis = millis();
          LogPrintln("OBD2: connected.");
          OBD2Started = 2;
        } else
        {
          OBD2.end();
          LogPrintln("OBD2: failed.");
          OBD2Started = 0;
          vTaskDelay(1000);
        }
      }
...
uint16_t OBD2EngineStarted()
{
  if(!OBD2Started == 2)
    return false;

  if(lastPidTime < OBD2_DEFAULT_TIMEOUT && PIDValues[PIDS_ENGINE_RPM] > 250) return true;
    else return false;
}

uint32_t OBD2PidTime()
{
  if(!OBD2Started)
    return 0xFFFFFFFF;
...
void OBD2PrintPIDS(uint16_t active)
{
  OBD2Print = active;
  EEPROMWriteByte(EEPROM_PRINT_PIDS, OBD2Print);
}

uint16_t OBD2PrintPIDS()
{
  return OBD2Print;
}
